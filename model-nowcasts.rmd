...
local({
	...
	screePlot =
		screeDf %>%
    	ggplot(.) +
    	geom_line(aes(x = factors, y = cum_pct_of_total)) +
		geom_col(aes(x = factors, y = pct_of_total)) +
    	labs(x = 'Factors', y = 'Cumulative % of Total Variance Explained') + 
	...
	zPlots =
		lapply(colnames(zDf) %>% .[. != 'obsDate'], function(x)
			dplyr::select(zDf, obsDate, x) %>%
				setNames(., c('obsDate', 'value')) %>%
				ggplot() +
				geom_line(aes(x = obsDate, y = value)) +
...
local({
	...
	gofDf =
		...
		dplyr::summarize(., MAE = mean(value), MSE = mean(value^2))
	...
	varFittedPlots =
		...
		purrr::imap(., function(x, i)
			ggplot(x) +
				geom_line(aes(x = obsDate, y = value, group = type, linetype = type, color = type)) +
				labs(
	...
	ef$nc$varFittedPlots <<- varFittedPlots
	...
	kfPlots =
		lapply(colnames(ef$nc$zDf) %>% .[. != 'obsDate'], function(.varname)
			dplyr::bind_rows(
				...
				ggplot(.) +
				geom_line(aes(x = obsDate, y = value, color = type), size = 1.5) + 
...
local({
	...
	wide$q$display =
		wide$q$base %>%
		{lapply(colnames(.) %>% .[. != 'obsDate'], function(.varname) {mn
