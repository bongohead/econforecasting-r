# Set Constants
```{r}
DIR = 'D:/Onedrive/__Projects/econforecasting-nowcast'
CONST_DIR = 'D:/Onedrive/__Projects/econforecasting' # Path to directory with constants.r and inputs.r - should include SQL DB info, SFTP info, etc.
PACKAGE_DIR = 'D:/Onedrive/__Projects/econforecasting/econforecasting' # Path to package with helper functions
DOC_PATH = 'D:/Onedrive/__Projects/econforecasting-nowcast/documentation/nowcast.rnw' # Path to documentation .RNW file
```
...
# Read constants
source(file.path(CONST_DIR, 'constants.r'))
...
local({
	...
	paramsDf = readxl::read_excel(file.path(CONST_DIR, 'inputs.xlsx'))
...
local({
	...
	flat$stat =
		ef$h$seasDf %>%
		dplyr::left_join(., ef$paramsDf[, c('varname', 'stat')], by = c('varname')) %>%
	...
	flatDf = purrr::imap_dfr(flat, function(x, transform) x %>% mutate(., transform = transform)) 
...
local({
	...
	timeDf =
		tibble(
			date = as.Date(c(pcaVariablesDf$obsDate[[1]], bigTDate, bigTauDate, bigTStarDate)),
			time = c('1', 'T', 'Tau', 'T*')
		)
...
local({
	...
	zPlots =
		purrr::imap(colnames(zDf) %>% .[. != 'obsDate'], function(x, i)
			dplyr::select(zDf, all_of(c('obsDate', x))) %>%
				setNames(., c('obsDate', 'value')) %>%
				ggplot() +
				geom_line(
					aes(x = obsDate, y = value),
...
local({
	...
	residPlot =
	    ...
	    as.data.frame(.) %>%
	    as_tibble(.) %>%
	    dplyr::bind_cols(obsDate = inputDf$obsDate, .) %>%
		tidyr::pivot_longer(., -obsDate) %>%
		ggplot(.) + 	
		geom_line(aes(x = obsDate, y = value, group = name, color = name), size = 1) +
	...
	fittedPlots =
    	...
    	fitted(.) %>%
		as_tibble(.) %>%
		dplyr::bind_cols(obsDate = inputDf$obsDate, ., type = 'Fitted Values') %>%
		dplyr::bind_rows(., ef$nc$zDf %>% dplyr::mutate(., type = 'Data')) %>%
		tidyr::pivot_longer(., -c('type', 'obsDate')) %>% 
		as.data.table(.) %>%
		split(., by = 'name') %>%
		purrr::imap(., function(x, i)
			ggplot(x) +
				geom_line(
					aes(x = obsDate, y = value, group = type, linetype = type, color = type),
					size = 1, alpha = 1.0
					) +
				labs(
...
local({
	...
	xDf = ef$nc$zDf %>% dplyr::select(., -obsDate) %>% dplyr::bind_cols(constant = 1, .)
	...
	fittedPlots =
	    ...
	    fitted(.) %>%
	    as_tibble(.) %>%
	    dplyr::bind_cols(obsDate = ef$nc$zDf$obsDate, ., type = 'Fitted Values') %>%
	...
	kfPlots =
		lapply(colnames(ef$nc$zDf) %>% .[. != 'obsDate'], function(.varname)
			dplyr::bind_rows(
				...
				ggplot(.) +
				geom_line(aes(x = obsDate, y = value, color = type), size = 1) + 
	...
	dfmMDf = 
		lapply(dfmVarnames, function(.varname) {
			inputDf =
				dplyr::inner_join(
					ef$nc$fDf,
					dplyr::select(ef$h$m$stat, obsDate, all_of(.varname)),
					by = 'obsDate'
				) %>%
				na.omit(.)
...
local({
	...
	# 			dplyr::inner_join(
	# 				fDf,
	# 				dplyr::select(ef$h$q$stat, obsDate, all_of(.varname)),
	# 				by = 'obsDate'
	# 			) %>%
	# 			na.omit(.)
	# 
...
local({
	...
	ef$ncpred0$m$stat <<- mDf
	ef$ncpred0$q$stat <<- qDf
})
```
...
local({
	...
	mDf =
		...
		{lapply(colnames(.) %>% .[. != 'obsDate'], function(.varname) {
			...
			transform = dplyr::filter(ef$paramsDf, varname == .varname)$stat
			
			fcDf = dplyr::select(., obsDate, .varname) %>% na.omit(.)
				
			histDf = tail(dplyr::filter(na.omit(ef$h$m$base[, c('obsDate', .varname)]), obsDate < min(fcDf$obsDate)), 1)
	...
	qDf =
		...
		{lapply(colnames(.) %>% .[. != 'obsDate'], function(.varname) {
			...
			histDf = tail(dplyr::filter(na.omit(ef$h$q$base[, c('obsDate', .varname)]), obsDate < min(fcDf$obsDate)), 1)
	...
	ef$ncpred0$m$base <<- mDf
	ef$ncpred0$q$base <<- qDf
})
```
...
local({
	...
	ef$ncpred1$m$base <<- mDf
	ef$ncpred1$q$base <<- qDf
})
```
...
local({
	...
	wide$m$base =
		list(ef$ncpred0$m$base, ef$ncpred1$m$base) %>%
		purrr::reduce(., function(x, y) dplyr::full_join(x, y, by = 'obsDate')) %>%
		dplyr::arrange(., obsDate)

	wide$q$base =
		list(ef$ncpred0$q$base, ef$ncpred1$q$base) %>%
		purrr::reduce(., function(x, y) dplyr::full_join(x, y, by = 'obsDate')) %>%
		dplyr::arrange(., obsDate)
	...
	wide$m$display =
		...
		{lapply(colnames(.) %>% .[. != 'obsDate'], function(.varname) {
			...
			transform = dplyr::filter(ef$paramsDf, varname == .varname)$display
			fcDf = dplyr::select(., obsDate, .varname) %>% na.omit(.)
...
## Save data to RDS and create documentation
```{r}
local({

	saveRDS(list(CONST = CONST, ef = ef), file = file.path(DIR, 'nowcast.rds'))
	setwd(str_replace(DOC_PATH, '/[^/]+$', ''))
	knitr::knit2pdf(DOC_PATH)
	file.copy(
		from = str_replace(DOC_PATH, '.rnw', '.pdf'),
