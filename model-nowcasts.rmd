...
RESET_ALL = FALSE
VINTAGE_DATE = as.Date('2021-03-15') # Sys.Date() # Date to pull data "as-of", for general use set this as Sys.Date(); otherwise set as an older date for backtesting
```
...
## Yahoo Finance - use last available data point as value
```{r}
local({
...
local({
	...
	# 			dplyr::select(., all_of(c('date', .varname)))
			...
			glmResult = lapply(seq(.75, 1, .05), function(.alpha) {
				cv =
					glmnet::cv.glmnet(
