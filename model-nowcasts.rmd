...
local({
	...
	flat$st =
		ef$h$seasDf %>%
		dplyr::left_join(., ef$paramsDf[, c('varname', 'st')], by = c('varname')) %>%
		...
		lapply(., function(x)
			x %>%
				...
				dplyr::mutate(
					.,
					value = {
						if (head(., 1)$st == 'base') value
						else if (head(., 1)$st == 'dlog') dlog(value)
						else if (head(., 1)$st == 'diff1') diff1(value)
						else if (head(., 1)$st == 'diff2') diff2(value)
						else if (head(., 1)$st == 'ma2') ma2(value)
						else if (head(., 1)$st == 'lma2') lma2(value)
						else if (head(., 1)$st == 'apchg') apchg(value)
						else stop ('Error')
						}
					)
			) %>%
