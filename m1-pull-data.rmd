...
VINTAGE_DATE = as.Date('2021-09-30')
```
...
local({
	...
	fredRes =
		...
		lapply(., function(x) {
			...
			# Get series data
			dataDf =
				getDataFred(x$sckey, CONST$FRED_API_KEY, .freq = x$freq, .returnVintages = TRUE, .vintageDate = as_date(with_tz(now(), tz = 'America/Chicago')), .verbose = TRUE) %>%
