...
local({
	...
	cmefiDf =
		...
		dplyr::anti_join(., m$h$source$fred, by = c('varname', 'date', 'freq')) %>%
		# Pivot
		dplyr::select(., -freq) %>%
...
local({
	...
	dfmVarnames = m$paramsDf %>% dplyr::filter(., nowcast == 'dfm.m') %>% .$varname
...
local({
	...
	dfmVarnames = m$paramsDf %>% dplyr::filter(., nowcast == 'dfm.q') %>% .$varname
...
local({
	...
	mDf =
		...
		{lapply(colnames(.) %>% .[. != 'date'], function(.varname) {
			...
			transform = dplyr::filter(m$paramsDf, varname == .varname)$st
			
			fcDf = dplyr::select(., date, .varname) %>% na.omit(.)
				
			histDf = tail(dplyr::filter(na.omit(m$h$m$ut[, c('date', .varname)]), date < min(fcDf$date)), 1)
	...
	qDf =
		...
		{lapply(colnames(.) %>% .[. != 'date'], function(.varname) {
			...
			histDf = tail(dplyr::filter(na.omit(m$h$q$ut[, c('date', .varname)]), date < min(fcDf$date)), 1)
...
local({
	...
	wide$m$ut =
		list(m$ncpred0$m$ut, m$ncpred1$m$ut) %>%
		purrr::reduce(., function(x, y) dplyr::full_join(x, y, by = 'date')) %>%
		dplyr::arrange(., date)

	wide$q$ut =
		list(m$ncpred0$q$ut, m$ncpred1$q$ut) %>%
		purrr::reduce(., function(x, y) dplyr::full_join(x, y, by = 'date')) %>%
		dplyr::arrange(., date)
	...
	wide$m$d1 =
		...
		{lapply(colnames(.) %>% .[. != 'date'], function(.varname) {
			...
			transform = dplyr::filter(m$paramsDf, varname == .varname)$d1
			fcDf = dplyr::select(., date, .varname) %>% na.omit(.)
