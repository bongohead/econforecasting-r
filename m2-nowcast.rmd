...
M1_PATH = 'D:/Onedrive/__Projects/econforecasting/model-outputs/[2021-07-26] m1.rds'
...
local({
	...
	zDfs =
		lapply(c(bigR - 1, bigR, bigR + 1), function(thisR)
			xDf[, 'date'] %>%
			dplyr::bind_cols(
				.,
	...
	factorWeightsDf =
		...
		dplyr::arrange(., order) %>%
		dplyr::select(., -order) %>%
		dplyr::select(., paste0('f', 1:(bigR + 1)))
