...
local({
    ...
    filePaths =
        seq(as.Date('2021-01-01'), to = add_with_rollback(Sys.Date(), months(-1)), by = '3 months') %>%
...
local({
    ...
    coefMat = dplyr::bind_rows(dplyr::bind_rows(coefMatsIdentity), dplyr::bind_rows(coefMatsEstimated))
    
    
    # Pass accumulator new exogenous variables each time
    resDf = purrr::accumulate(5:nrow(initDf), function(df, .row) {
        ...
        allCoefDf =
            ...
            dplyr::bind_cols(
                .,
                dplyr::transmute_if(df, is.numeric, function(x) ma2(x)) %>%
                    dplyr::rename_with(., function(x) paste0('ma2.', x))
                ) %>%
            dplyr::bind_cols(
                .,
                dplyr::transmute_if(df, is.numeric, function(x) lma2(x)) %>%
                    dplyr::rename_with(., function(x) paste0('lma2.', x))
                ) %>%
            .[.row, ]
        
        coefMat %>% dplyr::select(., all_of(endogVars))
