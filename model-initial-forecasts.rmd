...
local({
    ...
    ef$dnsLambda <<- optimLambda
    ef$dnsCoefs <<- dnsCoefs
    ef$dnsFitChart <<- dnsFitChart
    ef$dnsYieldCurveNamesMap <<- yieldCurveNamesMap
})
```
...
local({
    ...
    dnsCoefs = ef$dnsCoefs
    dnsLambda = ef$dnsLambda
    dnsYieldCurveNamesMap = ef$dnsYieldCurveNamesMap
    ...
    ef$e$forecasts$dns <<- dplyr::bind_rows(df1, df2)
})
```
...
local({
    ...
    ef$initialQualBaseline <<- initial 
})
```
...
local({
    ...
    coefMatsIdentity =
        list(
            gdp =
                tibble(gdp = 1, pce = -ss$pce/ss$gdp, pdi = -ss$pdi/ss$gdp, im = ss$im/ss$gdp, ex = -ss$ex/ss$gdp, govt = -ss$govt/ss$gdp),
        )
    ...
    eqn = 'pce = constant + ma2.dpi + pce.l1'
    
    yCoef = eqn %>% str_split_fixed(., '=', 2) %>% .[1, 1] %>% str_squish(.)
    xCoefs = eqn %>% str_split_fixed(., '=', 2) %>% .[1, 2] %>% str_split(., '\\+') %>% .[[1]] %>% str_squish(.)

    inputDf = histDfAll %>% dplyr::select(., date, all_of(c(yCoef, xCoefs))) %>% na.omit(.)
    
    yMat = inputDf %>% dplyr::select(., yCoef) %>% as.matrix(.)
    xMat = inputDf %>% dplyr::select(., xCoefs) %>% as.matrix(.)
    ...
    coefMat =
        ...
        as.data.frame(.) %>%
        as_tibble(.) %>%
        dplyr::bind_cols(!!yCoef := 1, .)
    
    
    # Now bind to single coefficient mat
    coefMatsEstimated =
