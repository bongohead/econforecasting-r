...
<<>>=
ef$h$q$stat %>% dplyr::select(., vsales, csent, advsales, ipi, vix, spy, cfnai) %>% dplyr::rename(., 'VehicleSales' = vsales, 'ConsumerSentiment' = csent, 'AdvSales' = advsales, 'IndustrialProd' = ipi, 'VIX' = vix, 'SP500' = spy, 'CFNAI' = cfnai) %>% corrr::correlate(.) %>% corrr::shave(.) %>% dplyr::select(., -CFNAI) %>% xtable::xtable(., caption = 'Correlation Between Major Leading Predictors') %>% print(., size = 'scriptsize')
@
...
ef$ncpred$q$display %>%
	tidyr::pivot_longer(., -obsDate, names_to = 'varname') %>%
    dplyr::inner_join(filter(ef$paramsDf, category != 'Output')[, c('varname', 'units', 'display', 'fullname')], ., by = 'varname') %>%
