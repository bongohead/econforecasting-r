...
M3_PATH = 'D:/Onedrive/__Projects/econforecasting/model-outputs/[2021-08-22] m3.rds'
...
local({
    ...
    eqnsList = list(
    	'pcegdmotor = constant + ma2.pid + ma2.ue + lma2.pcegdmotor + oil',
    	'pcegdfurnish = constant + ma2.pid + ma2.ue + lma2.pcegdfurnish',
    	'pcegdrec = constant + ma2.pid + ma2.ue + lma2.pcegdrec',
    	'pcegdrec = constant + ma2.pid + ma2.ue + lma2.pcegdrec',
    	'pcegdother = constant + ma2.pid + ma2.ue + lma2.pcegdother',
    	'pcegnfood = constant + ma2.pid + ma2.ue + lma2.pcegnfood',
    	'pcegnclothing = constant + ma2.pid + ma2.ue + lma2.pcegnclothing',
    	'pcegngas = constant + ma2.pid + ma2.ue + lma2.pcegngas',
    	'pcegnother = constant + ma2.pid + ma2.ue + lma2.pcegnother',
    	'pceshousing = constant + ma2.pid + ma2.ue + lma2.pceshousing',
    	'pceshealth = constant + ma2.pid + ma2.ue + lma2.pceshealth',
    	'pcestransport = constant + pid + ma2.ue + lma2.pcestransport',
    	'pcesrec = constant + ma2.pid + ma2.ue + lma2.pcesrec',
    	'pcesfood = constant + ma2.pid + ma2.ue + lma2.pcesfood',
    	'pcesfinal = constant + ma2.pid + ma2.ue + lma2.pcesfinal',
    	'pcesother = constant + ma2.pid + ma2.ue + lma2.pcesother',
    	'pcesnonprofit = constant + ma2.pid + ma2.ue + lma2.pcesnonprofit',
    	
    	'pdi = constant + pdin + pdir',
    	'pdinstruct = constant + ma2.pce + spy + lma2.pdinstruct + lma2.t10y',
    	'pdinequip = constant + ma2.pce + spy + lma2.pdinequip + lma2.t10y',
    	'pdinip = constant + ma2.pce + spy + lma2.pdinip + lma2.t10y',
    	'pdir = constant + ma2.pce + spy + lma2.pdir + lma2.mort30y + ma2.hpi',
    	...
    	'vsales = constant + lma2.vsales + pce + oil',
    	'emp = constant + ue + gdp',
...
local({
	...
	predbase0m = imap(m$csm$predbase0, function(df, scenarioname) {
		...
		varnames = filter(p$variablesDf, core_structural == 'core.endog' & freq == 'm')$varname
		exogVarnames  = filter(p$variablesDf, core_structural == 'core.exog' & freq == 'm')$varname
		...
		lapply(varnames, function(.varname) {
			...
			df %>%
				...
				# dplyr::left_join(
				# 	.,
				# 	m$ncpred$base$m %>% dplyr::transmute(., date, nc = .[[.varname]]),
				# 	by = 'date'
				# 	)
			...
			dplyr::left_join(., m$qual$pred[[scenarioname]]$base, by = 'date') %>%
			dplyr::arrange(., date) %>%
			# Calculations
			# dplyr::mutate(
			# 	.,
