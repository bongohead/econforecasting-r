...
M3_PATH = 'D:/Onedrive/__Projects/econforecasting/model-outputs/[2021-07-30] m3.rds'
...
local({
	...
	forecasts = lapply(m$csm$exogDfs, function(exogDf) {
	    ...
	    pred = resDf %>% dplyr::filter(., date >= m$csm$startDate) %>% dplyr::select(., all_of(c('date', endogVars)))
