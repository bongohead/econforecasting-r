...
local({
    ...
    eqnsList =
        list(
            pce = 'pce = constant + pid + pce.l1 + ue',
...
local({
	...
	flat0 =
		...
		purrr::imap(., function(x, i) {
			...
			purrr::map_dfr(c('st', 'd1', 'd2'), function(.form) {
	...
	base = list(q = m$csm$predbaseq, m = m$csm$predbase0m)
	forms = c('st', 'd1', 'd2')
	
	
	# lapply(names(m$csm$predbaseq) %>% setNames(., .), function(scenarioname) {
	# 	base = list(q = m$csm$predbaseq[[scenarioname]], m = m$csm$predbase0m[[scenarioname]])
	# 	
	# 	otherForms = lapply(c('st', 'd1', 'd2') %>% setNames(., .), function(form) {
	# 		
	# 
	# 	
	# 	})
	# 
	# res = purrr::imap_dfr(base, function(df) {
	# 		
	# 		
	# 		
	# 	})
