...
\usepackage{fancyhdr}
\usepackage{appendix}
\graphicspath{{\Sexpr{file.path(DIR, 'modules', 'nowcast-model', 'documentation-images')}}}
...
<<echo=F,results='hide',message=F, warning=F>>=
  ...
  knitr::opts_chunk$set(
    echo = F, results = 'asis',
    root.dir = file.path(DIR, 'modules', 'nowcast-model'),
    fig.path = file.path(DIR, 'logs', 'latex-figures'),
    cache.path = file.path(DIR, 'logs', 'latex-cache'),
...
<<>>=
	hist$wide_last$q$st %>% 
  	select(., vsales, csent, advsales, ipi, vix, spy, cfnai) %>%
  	rename(., 'GDP' = gdp, 'VehicleSales' = vsales, 'ConsumerSentiment' = csent, 'AdvSales' = advsales, 'IndustrialProd' = ipi, 'VIX' = vix, 'SP500' = spy) %>%
  	corrr::correlate(.) %>%
  	corrr::shave(.) %>%
  	select(., -CFNAI) %>%
  	xtable::xtable(., caption = 'Correlation Between Major Leading Predictors') %>%
