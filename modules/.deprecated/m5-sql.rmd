...
local({
	...
	# if (RESET_ALL == TRUE) {
	# 	
	# 	DBI::dbExecute(conn, 'TRUNCATE csm_releases')
	# 	DBI::dbExecute(conn, 'TRUNCATE csm_values')
	# 	DBI::dbExecute(conn, 'TRUNCATE csm_history')
	# }
...
local({
	...
	if (RESET_ALL) {
		DBI::dbExecute(db, 'DROP TABLE IF EXISTS csm_releases CASCADE')
		DBI::dbExecute(db, '
...
local({
	...
	if (RESET_ALL) {
		DBI::dbExecute(db, 'DROP TABLE IF EXISTS csm_params CASCADE')
		DBI::dbExecute(db, '
...
local({
	...
	if (RESET_ALL) {
		DBI::dbExecute(db, 'DROP TABLE IF EXISTS csm_tstypes CASCADE')
		...
		DBI::dbExecute(db, 'CREATE INDEX tstype_ix ON csm_tstypes (tstype);')
		DBI::dbExecute(db, 'CREATE INDEX fctype_ix ON csm_tstypes (fctype);')
	}
...
local({
	...
	if (RESET_ALL) {
		DBI::dbExecute(db, 'DROP TABLE IF EXISTS csm_tsvalues')
	...
	resVals$hist =
		...
		sapply(., function(x)
			econforecasting::createInsertQuery(x, 'csm_tsvalues', onConflict) %>%
				DBI::dbExecute(db, .)
	...
	if (any(is.null(unlist(resVals)))) stop('Error with one or more SQL queries')
	resVals %>% imap(., function(x, i) paste0(i, ': ', x)) %>% paste0(., collapse = '\n') %>% cat(.)
	print(sum(unlist(resVals)))
})
```
