...
local({
  ...
  # @param returnAll: (boolean) FALSE by default. If FALSE, will return only the MAPE (useful for optimization).
  # Otherwise, will return a list of fitted values.
  
  getDnsFit = function(lambda, returnAll = FALSE) {
    ef$history %>%
      purrr::keep(., ~ .$varname[[1]] %in% yieldCurveNamesMap$varname) %>%
      dplyr::bind_rows(.) %>%
      dplyr::filter(., obsDate >= add_with_rollback(Sys.Date(), months(-3))) %>%
      dplyr::right_join(., yieldCurveNamesMap, by = 'varname') %>%
      dplyr::mutate(., f1 = 1, f2 = (1-exp(-1 * lambda * ttm))/(lambda * ttm), f3 = f2 - exp(-1 * lambda * ttm)) %>%
  ...
  optimLambda = optimize(getDnsFit, returnAll = FALSE, interval = c(-1, 1), maximum = FALSE)$minimum
  
  dnsCoefs =
    getDnsFit(optimLambda, returnAll = TRUE) %>%
  ...
  fittedCurve =
    ...
    dplyr::mutate(
      ...
      netYield = (1 + yield/100)^(ttm/12)
      )
  
  
  fittedCurve %>% dplyr::mutate(., futNetYield = dplyr::lead(netYield, 240)/netYield, futYield = (futNetYield^(12/240) - 1) * 100) %>% dplyr::filter(., ttm < 120) %>% ggplot(.) + geom_line(aes(x = ttm, y = futYield))
  
    ttm0 = 3
  fittedCurve %>% dplyr::mutate(., futYield = (dplyr::lead(netYield, 3)/netYield - 1) * 100)
  ...
  yieldCurveNamesMap$ttm %>%
    lapply(., function(yttm)
      fittedCurve %>%
        dplyr::mutate(
          .,
          futNetYield = dplyr::lead(netYield, yttm)/netYield,
          futYield = (futNetYield^(12/yttm) - 1) * 100
          ) %>%
    ...
    dplyr::mutate(
      .,
      forecastDate = add_with_rollback(curDate, months(ttm))
      ) %>%
    dplyr::transmute(., varname, forecastDate, value = futYield)
