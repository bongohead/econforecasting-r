...
local({
  ...
  yieldCurveNamesMap =
    ...
    .$varname %>%
    unique(.) %>%
    purrr::keep(., ~ str_sub(., 1, 4) == 'tips' & str_length(.) == 7) %>%
    tibble(varname = .) %>%
    dplyr::mutate(., ttm = as.numeric(str_sub(varname, -3, -2)) * ifelse(str_sub(varname, -1) == 'y', 12, 1)) %>%
  	dplyr::mutate(., varname = str_replace(varname, 'tips', 'inf')) %>%
  	# Add additional varnames
  	dplyr::bind_rows(
  		.,
  		tibble(
  			varname = c('inf02y', 'inf01y'),
  			ttm = c(24, 12)
  			)
  	)
  
  # Get data frame of inflation expectation by subtracting TIPS from Treasury yields 
  spreadDf =
	ef$historyDf %>%
    dplyr::filter(
        .,
        varname %in% c(str_replace(yieldCurveNamesMap$varname, 'inf', 'tips'), str_replace(yieldCurveNamesMap$varname, 'inf', 't')) & freq == 'd'
    ) %>%
    dplyr::mutate(
    	.,
    	inf = str_sub(varname, 0, ifelse(str_detect(varname, 'tips'), 4, 1)),
    	ttm = str_sub(varname, -3, -1)
    	) %>%
  	dplyr::select(., -varname, -freq) %>%
  	tidyr::pivot_wider(., names_from = inf, values_from = value) %>%
  	na.omit(.) %>%
  	dplyr::mutate(., varname = paste0('inf', ttm), value = t - tips) %>%
  	dplyr::select(., -t, -tips, -ttm) %>%
  	dplyr::bind_rows(
  		.,
  		# Clone cleveland fed forecasts across 1m
  		dplyr::filter(., varname == 'inf05y') %>%
  			dplyr::mutate(., varname = 'inf02y') %>%
  			dplyr::mutate(., value = ef$forecast$cle[[24, 'value']]),
  		dplyr::filter(., varname == 'inf05y') %>%
  			dplyr::mutate(., varname = 'inf01y') %>%
  			dplyr::mutate(., value = ef$forecast$cle[[12, 'value']]),
  		dplyr::filter(., varname == 'inf05y') %>%
  			dplyr::mutate(., varname = 'inf06m') %>%
  			dplyr::mutate(., value = ef$forecast$cle[[6, 'value']]),
  		dplyr::filter(., varname == 'inf05y') %>%
  			dplyr::mutate(., varname = 'inf03m') %>%
  			dplyr::mutate(., value = ef$forecast$cle[[3, 'value']]),
  		dplyr::filter(., varname == 'inf05y') %>%
  			dplyr::mutate(., varname = 'inf01m') %>%
  			dplyr::mutate(., value = ef$forecast$cle[[1, 'value']])
  	)
  ...
  df =
    ...
    dplyr::transmute(
      ...
      fcname = 'tip', vintageDate = trainDf %>% .$obsDate %>% max(.)
      ) %>%
  	dplyr::filter(., varname == 'inf01y') %>%
  	dplyr::mutate(., varname == 'inf') 
  
  df %>%
  	ggplot(.) + geom_line(aes(x = obsDate, y = value))
