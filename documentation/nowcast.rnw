...
<<>>=
rds$ef$h$q$stat %>% dplyr::select(., vsales, csent, advsales, ipi, vix, spy, cfnai) %>% dplyr::rename(., 'VehicleSales' = vsales, 'ConsumerSentiment' = csent, 'AdvSales' = advsales, 'IndustrialProd' = ipi, 'VIX' = vix, 'SP500' = spy, 'CFNAI' = cfnai) %>% corrr::correlate(.) %>% corrr::shave(.) %>% dplyr::select(., -CFNAI) %>% xtable::xtable(., caption = 'Correlation Between Major Leading Predictors') %>% print(., size = 'scriptsize')
@
...
rds$ef$ncpred$q$display %>%
	tidyr::pivot_longer(., -obsDate, names_to = 'varname') %>%
    dplyr::inner_join(filter(rds$ef$paramsDf, category != 'Output')[, c('varname', 'units', 'display', 'fullname')], ., by = 'varname') %>%
    dplyr::mutate(., fullname = ifelse(display != 'base', paste0(fullname, ' \\textit{\\footnotesize\\textcolor{gray}{(', display, ')}}'), fullname)) %>%
    dplyr::mutate(., fullname = str_replace(fullname, coll('&'), '\\&')) %>%
	dplyr::mutate(., obsDate = paste0(year(obsDate), 'Q', quarter(obsDate))) %>%
	dplyr::select(., -varname, -display, -units) %>%
	tidyr::pivot_wider(., values_from = value, names_from = obsDate) %>%
	dplyr::rename(., 'Variable' = fullname) %>%
...
@
<<>>=
rds$ef$ncpred$m$display %>%
    tidyr::pivot_longer(., -obsDate, names_to = 'varname') %>%
    dplyr::inner_join(filter(rds$ef$paramsDf, category != 'Output')[, c('varname', 'units', 'display', 'fullname')], ., by = 'varname') %>%
    dplyr::mutate(., fullname = ifelse(display != 'base', paste0(fullname, ' \\textit{\\footnotesize\\textcolor{gray}{(', display, ')}}'), fullname)) %>%
    dplyr::mutate(., fullname = str_replace(fullname, coll('&'), '\\&')) %>%
    dplyr::mutate(., obsDate = paste0(year(obsDate), 'M', month(obsDate))) %>%
    dplyr::select(., -varname, -display, -units) %>%
    tidyr::pivot_wider(., values_from = value, names_from = obsDate) %>%
    dplyr::rename(., 'Variable' = fullname) %>%
