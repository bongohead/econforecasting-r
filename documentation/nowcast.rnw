...
<<>>=
	rds$CONST$PARAMS %>%
    	...
    	xtable::xtable(., caption = 'Imported Monthly Data') %>%
...
<<>>=
	rds$CONST$PARAMS %>%
    	...
    	dplyr::select(., fullname, stat) %>%
    	setNames(., c('Variable', 'Stationary Form')) %>%
    	xtable::xtable(., caption = 'Imported Quarterly Data') %>%
    	print(., size = 'scriptsize')
@
...
<<>>=
	rds$ef$nc$screeDf %>%
    	xtable::xtable(., caption = 'Factor Selection Process') %>%
    	print(.)
@
