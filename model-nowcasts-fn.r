testFunction = function(VINTAGE_DATE = Sys.Date()) {## ---------------------------------------------------------------------------------------------------------------------------------
# General purpose
...
local({
	...
	fDf = dplyr::bind_rows(kSmooth, tail(kFitted, 1), kForecast %>% dplyr::mutate(., f1 = mean(dplyr::filter(ef$nc$zDf, date < '2020-03-01')$f1)))
		...
		purrr::imap_dfr(function(x, .freq)
			x %>% purrr::imap_dfr(function(df, .form)
				df %>%
					...
					dplyr::mutate(., freq = .freq, form = .form, vdate = Sys.Date())
				)
			) %>%
		na.omit(.)
