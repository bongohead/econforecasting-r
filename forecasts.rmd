...
local({
  ...
  df =
    tribble(
      ~ fcname, ~ frequency, ~ fullname, ~ cmefi,
...
local({
	...
	query =
	  econforecasting::createInsertQuery(
	    ef$forecastnames,
	    'fc_forecastnames',
	    'ON CONFLICT ON CONSTRAINT fc_forecastnames_fcname DO UPDATE SET frequency = EXCLUDED.frequency, fullname = EXCLUDED.fullname, cmefi = EXCLUDED.cmefi'
	    )
