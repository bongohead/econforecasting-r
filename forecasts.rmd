...
local({
  ...
  df =
    tribble(
      ~ fcname, ~ freq, ~ fullname, ~ cmefi,
      'atl', 'q', 'Atlanta Fed GDPNow', F,
      'stl', 'q', ' St. Louis Fed Economic News Index', F,
      'nyf', 'q', 'New York Fed Staff Nowcast', F,
      'spf', 'q', 'Philadelphia Fed Survey of Professional Forecasts', F,
      'wsj', 'q', 'WSJ Consensus Survey', F,
      'wfc', 'q', 'Wells Fargo', F,
      'fnm', 'q', 'Fannie Mae', F,
      'gsu', 'q', 'Georgia State', F,
      'cbo', 'q', 'Congressional Budget Office', F,
      'cme', 'q', 'Consensus Futures-Implied Forecast', T,
      'dns', 'm', 'CMEFI Interest Rate Model', T
      )
...
local({
	...
	query =
	  econforecasting::createInsertQuery(
	    ef$forecastnames,
	    'fc_forecastnames',
	    'ON CONFLICT ON CONSTRAINT fc_forecastnames_fcname DO UPDATE SET freq = EXCLUDED.freq, fullname = EXCLUDED.fullname, cmefi = EXCLUDED.cmefi'
	    )
