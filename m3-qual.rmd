...
local({
    ...
    downside$ffr =
    	...
    	dplyr::mutate(
    		.,
    		value = ifelse(date >= as.Date('2022-06-01') & date <= as.Date('2022-12-01'), value - .1, value),
			value = ifelse(date >= as.Date('2023-01-01') & date <= as.Date('2024-01-01'), value - .05, value)
    	) %>%
        dplyr::transmute(., freq, form, date, value)
    ...
    downside$mort15yt10yspread =
    	# Get change in tdns1
    	dplyr::bind_rows(
    		p$variables$tdns1$h$d1$m,
    		downside$tdns1 %>% dplyr::select(., -form)
    		) %>%
    	dplyr::mutate(., value = (value - dplyr::lag(value, 1))) %>%
    	# Scale to appropriate change desired in mort15y10yspread
    	dplyr::mutate(., value = value * -1/4) %>%
    ...
    m$qual$predraw$downside <<- downside
})
```
