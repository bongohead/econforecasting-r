...
M2_PATH = 'D:/Onedrive/__Projects/econforecasting/model-outputs/[2021-07-19] m2.rds'
...
local({
	...
	df =
		lapply(m$qual$predbase, function(scenario)
			lapply(purrr::transpose(m$dnsYieldCurveNamesMap), function(y)
					scenario %>%
						dplyr::transmute(
							.,
							date,
	...
	x =
		...
		dplyr::mutate(
			.,
			t01m = ffr + tdns1 + tdns2 * (1-exp(-1 * lambda * t))/(lambda * t)
		)
...
## Combine With Nowcasts To Get SEM.EXOG Forecasts
```{r}
local({
